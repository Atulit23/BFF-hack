...
import AdressCards from "./AdressCards.mjs";
import cartcards from "./cartcards.mjs";
...
const url =
  "mongodb+srv://atulitgaur:sanjayashaS28@cluster0.zbmbdhh.mongodb.net/bffhackathon-customer";
...
app.post("/address", (req, res) => {
  ...
  AdressCards.create(emailCard, (err, data) => {
    if (err) {
      res.status(500).send(err);
    } else {
...
app.post("/cart", (req, res) => {
  const emailCard = req.body;
  cartcards.create(emailCard, (err, data) => {
    if (err) {
      res.status(500).send(err);
    } else {
      res.status(201).send(data);
