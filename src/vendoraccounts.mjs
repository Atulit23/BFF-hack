...
import VendorDetailCards from "./vendordetailcards.mjs";
...
const url =
  "mongodb+srv://atulitgaur:sanjayashaS28@cluster0.zbmbdhh.mongodb.net/bffhackathon-vendor";
...
app.post("/vendordetails", (req, res) => {
  ...
  VendorDetailCards.create(emailCard, (err, data) => {
    if (err) {
      res.status(500).send(err);
    } else {
...
app.get("/vendordetails", (req, res) => {
  VendorDetailCards.find((err, data) => {
    if (err) {
      res.status(500).send(err);
    } else {
      res.status(200).send(data);
